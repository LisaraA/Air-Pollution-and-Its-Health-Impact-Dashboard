...
# Columns the dashboard actually uses — everything else is dropped at load time
KEEP_COLS = [
...
# Row-level summary values that inflate counts and add no per-city/per-country signal
SUPRA           = ["All Countries",
                   "European Environment Agency Member Countries",
                   "European Union Countries"]
AGE_EXCLUDE     = [">= 0 years of age"]          # all-ages rollup
OUTCOME_EXCLUDE = ["All causes"]                  # all-outcomes rollup

@st.cache_data
def load_data():
    _dir = os.path.dirname(os.path.abspath(__file__))
    df = pd.read_csv(os.path.join(_dir, "DataExtract.csv"))

    # Normalise column names (collapse internal double-spaces, strip edges)
    df.columns = df.columns.str.replace(r"\s+", " ", regex=True).str.strip()

    # Drop unused columns
    df = df[[c for c in KEEP_COLS if c in df.columns]]

    # Drop summary / aggregate rows
    df = df[~df["Country Or Territory"].isin(SUPRA)]
    df = df[~df["Description Of Age Group"].isin(AGE_EXCLUDE)]
    df = df[~df["Outcome"].isin(OUTCOME_EXCLUDE)]

    # Drop rows with zero health burden — city/pollutant/outcome combos
    # with no recorded signal that would distort aggregations
    df = df[df["Value"] > 0]
...
with st.sidebar:
    ...
    pool_for_poll = (
        df_raw[df_raw["Outcome"].isin(sel_outcome)] if sel_outcome else df_raw
    )
    pollutant_opts = sorted(pool_for_poll["Air Pollutant"].unique().tolist())
    # Drop any previously selected pollutants that are no longer valid
    sel_poll = st.multiselect(
        "Air Pollutant",
        options=pollutant_opts,
        default=[p for p in st.session_state.get("_prev_poll", []) if p in pollutant_opts],
        key="poll_ms",
    )
    st.session_state["_prev_poll"] = sel_poll

    # ── Step 3: Age Group — restricted to what exists for selected outcomes
    #           AND selected pollutants 
    AGE_EXCLUDE_SIDEBAR = [">= 30 years of age"]
    pool_for_age = pool_for_poll.copy()
    if sel_poll:
        pool_for_age = pool_for_age[pool_for_age["Air Pollutant"].isin(sel_poll)]
    age_opts = sorted(
        pool_for_age.loc[
            ~pool_for_age["Description Of Age Group"].isin(AGE_EXCLUDE_SIDEBAR),
            "Description Of Age Group",
        ].unique().tolist()
    )
    sel_age = st.multiselect(
        "Age Group",
        options=age_opts,
        default=[a for a in st.session_state.get("_prev_age", []) if a in age_opts],
        key="age_ms",
    )
...
# Apply Filter (no filter - show all)
...
def _no_data_msg():
    st.markdown("""
    <div class='insight-box' style='border-left-color:#f7914f; margin-top:24px;'>
    <strong>&#9888;&#65039; No data for this filter combination.</strong><br>
    The selected Health Outcome, Air Pollutant, and Age Group have no overlapping records.
    Try removing one filter — each dropdown already shows only options compatible with
    what you selected above it.
    </div>
    """, unsafe_allow_html=True)
...
with tab2:
    ...
    if ad_country.empty:
        ...
        <div class='insight-box'>
        <strong>💡 Key Finding — Balkans dominate the mortality ranking:</strong>
        Apply fewer filters to see the full cross-country comparison and insight.
        </div>
        """, unsafe_allow_html=True)
...
with tab4:
    ...
    # Tab 5 — Death Vs Disability
    
with tab5:

    if yll_country.empty:
        _no_data_msg()
    else:
